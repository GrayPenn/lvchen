...
<title>My JSP 'houselist.jsp' starting page</title>
...
<script type="text/javascript">
	...
	function search() {
		alert("调用成功");
		$("#submitForm").attr("action", "HouseListServlet?page=" + 1)
				.submit();
	}
	...
	function batchDel() {
		...
		if (allIDCheck.length > 0) {
			...
			if (confirm("您确定要批量删除这些记录吗？")) {
				...
				//请求的URL
				var url = "HouseListServlet?allIDCheck=" + allIDCheck;
	...
	function jumpNormalPage(page) {
		alert("HouseListServlet");
		$("#submitForm").attr("action", "HouseListServlet?page=" + page)
				.submit();
	}
	...
	function jumpInputPage(totalPage) {
		...
		if ($("#jumpNumTxt").val() != '') {
			...
			$("#submitForm").attr("action",
					"HouseListServlet?page=" + pageNum).submit();
		} else {
			// “跳转页数”为空
			art.dialog({
				icon : 'error',
				...
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%"
						...
						<tr>
							...
							<c:if test="${houselist[time-1].hid!=null}">
								<tr>
									<td><input type="checkbox" name="IDCheck"
										value="${houselist[time-1].hid}" class="acb" /></td>
									<td>${houselist[time-1].hid}</td>
									<td>${houselist[time-1].hlocation}</td>
									<td><img alt="" src="${houselist[time-1].hcei} " width="60px" heigth="60px"></td>
									<td>${houselist[time-1].haddr}</td>
									
									<c:if test="${houselist[time-1].hstatu==0}">
									<td>拒绝</td>
									</c:if>
									<c:if test="${houselist[time-1].hstatu==1}">
									<td>通过</td>
									</c:if>
									<td>${houselist[time-1].htype}</td>
									<td><a href="house_edit.jsp?fyID=14458625716623"
										class="edit">编辑</a> <a href="ToDelComment?cid=" 
										+${houselist[time-1].hid}>删除</a></td>
