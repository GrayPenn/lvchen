...
<link rel="stylesheet" type="text/css" href="/lvchen/admin/style/authority/jquery.fancybox-1.3.4.css" media="screen"></link>
...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("a.edit").fancybox({
	        ...
	        'showCloseButton' : false,
	        'onClosed' : function() { 
	        	window.location.href = 'hostedit.jsp';
	        }
	    });
	});
	/** 用户角色   **/
	...
	/** 模糊查询来电用户  **/
	function search(){
		$("#submitForm").attr("action", "../HostListServlet" ).submit();
	}
...
// 				return;
	...
	function jumpNormalPage(page){
		$("#submitForm").attr("action", "../HostListServlet?page=" + page).submit();
	}
	...
	function jumpInputPage(totalpage){
		...
		if($("#jumpNumTxt").val() != ''){
			...
			$("#submitForm").attr("action", "../HostListServlet?page=" + pageNum).submit();
		}else{
			// “跳转页数”为空
			art.dialog({icon:'error', title:'友情提示', drag:false, resize:false, content:'请输入合适的页数，\n自动为您跳到首页', ok:true,});
			$("#submitForm").attr("action", "../HostListServlet?page=" + 1).submit();
		}
	}
</script>
<style>
					...
					<div id="box_border">
						...
						<div id="box_center">
							...
							<select name="fangyuanEntity.fyStatus" id="fyStatus" class="ui_select01">
                                <option value="">--请选择--</option>
                                <option value="26">待审核</option>
                                <option value="25">通过</option>
                                <option value="13">修改</option>
					...
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach items="${user}" var="u">
							<tr>
								...
								<td><input type="checkbox" name="IDCheck" value="14458625716623" id="${u.uid }" class="acb" /></td>
								<td>${u.uid }</td>
								<td></td>
								<td><img alt="" src="admin/${u.uicon}"></td>
								<td>${u.uname }</td>
								<td>${u.uphone }</td>
								<td>${u.uaccount }</td>
								<td>u.hid</td>
								...
								<c:if test="${u.ustatu==0}">
					             房东：是
								        			</c:if>
														<c:if test="${u.ustatu==1}">
								           	房东：否
								        			</c:if>
													</td>
								<td>
									<a href="admin/houseedit.jsp?uid=${u.uid }" class="edit">编辑</a> 
